...
function Kelas7PerpusContent() {
  ...
  useEffect(() => {
    fetchKelas7Books(currentPage);
  }, [fetchKelas7Books, currentPage]);

  const handlePageChange = (page: number) => {
    const params = new URLSearchParams(searchParams.toString());
    params.set('page', page.toString());
    router.push(`/kelasVII_perpus?${params.toString()}`);
  };
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'ArrowLeft') {
        ...
        if (kelas7Pagination && currentPage < kelas7Pagination.lastPage) {
      ...
      <main className="pt-24 px-4 sm:px-8 flex-grow flex flex-col pb-8">
        <div className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div className="flex flex-col gap-1">
            <p className="text-xl font-semibold font-poppins">Buku Kelas VII</p>
            <p className="text-sm text-gray-500">Menampilkan {displayBooks.length} buku</p>
          </div>
          <div className="flex flex-wrap items-center gap-3">
            <FilterCheckbox books={kelas7Books || []} onFilterChange={setActiveFilters} />
        ...
        {kelas7Pagination && (
          <div className="mt-8 flex justify-center">
            <Pagination
              ...
              totalPages={kelas7Pagination.lastPage || 1}
