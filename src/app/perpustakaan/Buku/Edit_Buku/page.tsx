...
function Page() {
    ...
    useEffect(() => {
        if (id) {
            fetchBookById(id).then((data: any) => {
                ...
                // Pastikan URL cover benar
                setExistingCover(data.cover ? `http://localhost:8000/storage/${data.cover}` : '/assets/default-cover.png');
    
                const fileName = data.isi.split('/').pop(); // Mengambil nama file dari path 'books/{Namafilenya}'
                setPdfFileName(fileName || ''); // Menyimpan nama file PDF
            }).catch((err: any) => {
    ...
    <div className="border border-gray-300 rounded-lg p-6 bg-gray-50 relative">
        {coverFile ? (
            // Jika pengguna mengunggah gambar baru, tampilkan gambar dari file lokal
            <img
            ...
            // Jika tidak ada file baru, tampilkan gambar dari database atau default
            <Image
...
<div className="relative">
                        ...
                        <div className="border border-gray-300 rounded-lg flex items-center justify-center p-6 bg-gray-50 cursor-pointer">
                            <Image
                                ...
                                height={45}
                            />
                            {!pdfFile && pdfFileName && <p className="mt-2 text-gray-700">{pdfFileName}</p>} {/* Menampilkan nama file PDF dari database */}
                            <input
                                type="file"
                                ...
                                className="absolute top-0 left-0 w-full h-full opacity-0 cursor-pointer"
                            />
                            {pdfFile && <p className="mt-2 text-gray-700">{pdfFile.name}</p>} {/* Menampilkan nama file PDF yang diunggah */}
                        </div>
                    </div>
