...
function Kelas5PerpusContent() {
  ...
  const handlePageChange = (page: number) => {
    ...
    router.push(`/kelasV_perpus?${params.toString()}`);
  };
  ...
  useEffect(() => {
    if (kelas5Books) {
      const filteredBooks = kelas5Books.filter((book: any) => {
      ...
      const processedBooks = filteredBooks.map((book: any) => {
        ...
        return {
          ...
          judul: book.judul,
          cover: coverUrl,
          path: `/kelasV_perpus/buku5?id=${book.id}`,
  ...
  }, [kelas5Books, sortOption, activeFilters]);
      ...
      <main className="pt-24 px-4 sm:px-8 flex-grow flex flex-col pb-8">
        <div className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div className="flex flex-col gap-1">
            <p className="text-xl font-semibold font-poppins">Buku Kelas V</p>
            <p className="text-sm text-gray-500">Menampilkan {displayBooks.length} buku</p>
          </div>
          <div className="flex flex-wrap items-center gap-3">
            <FilterCheckbox books={kelas5Books || []} onFilterChange={setActiveFilters} />
