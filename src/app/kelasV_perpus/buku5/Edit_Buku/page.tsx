...
function Page() {
    ...
    const { fetchKelas5BookById, updateKelas5Book, loading, error } = useBook();
    ...
    useEffect(() => {
        if (id) {
            fetchKelas5BookById(id).then((data: any) => {
                setJudul(data.judul);
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        try {
            await updateKelas5Book(id, formData);
            setShowNotification(true);
            router.push(`/kelasV_perpus`);
        } catch (err) {
