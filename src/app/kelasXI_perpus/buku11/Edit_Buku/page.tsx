...
function Page() {
    ...
    const { fetchKelas11BookById, updateKelas11Book, loading, error } = useBook();
    ...
    useEffect(() => {
        if (id) {
            fetchKelas11BookById(id).then((data: any) => {
                setJudul(data.judul);
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        try {
            await updateKelas11Book(id, formData);
            setShowNotification(true);
            router.push(`/kelasXI_perpus`);
        } catch (err) {
