...
function Page() {
    ...
    const { fetchKelas8BookById, updateKelas8Book, loading, error } = useBook();
    ...
    useEffect(() => {
        if (id) {
            fetchKelas8BookById(id).then((data: any) => {
                setJudul(data.judul);
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        try {
            await updateKelas8Book(id, formData);
            setShowNotification(true);
            router.push(`/kelasVIII_perpus`);
        } catch (err) {
