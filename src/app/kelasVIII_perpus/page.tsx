...
function Kelas8PerpusContent() {
  useAuthMiddleware();
  const router = useRouter();
  const searchParams = useSearchParams();
  const { user } = useAuth();
  const { kelas8Books, kelas8Pagination, loading, error, fetchKelas8Books } = useBook();
  ...
  useEffect(() => {
    fetchKelas8Books(currentPage);
  }, [fetchKelas8Books, currentPage]);

  const handlePageChange = (page: number) => {
    const params = new URLSearchParams(searchParams.toString());
    params.set('page', page.toString());
    router.push(`/kelasVIII_perpus?${params.toString()}`);
  };
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'ArrowLeft') {
        ...
        if (kelas8Pagination && currentPage < kelas8Pagination.lastPage) {
  ...
  }, [currentPage, kelas8Pagination]);

  useEffect(() => {
    if (kelas8Books) {
      const filteredBooks = kelas8Books.filter((book: any) => {
      ...
      const processedBooks = filteredBooks.map((book: any) => {
        ...
        return {
          ...
          judul: book.judul,
          cover: coverUrl,
          path: `/kelasVIII_perpus/buku8?id=${book.id}`,
  ...
  }, [kelas8Books, sortOption, activeFilters]);
      ...
      <main className="pt-24 px-4 sm:px-8 flex-grow flex flex-col pb-8">
        <div className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div className="flex flex-col gap-1">
            <p className="text-xl font-semibold font-poppins">Buku Kelas VIII</p>
            <p className="text-sm text-gray-500">Menampilkan {displayBooks.length} buku</p>
          </div>
          <div className="flex flex-wrap items-center gap-3">
            <FilterCheckbox books={kelas8Books || []} onFilterChange={setActiveFilters} />
        ...
        {kelas8Pagination && (
          <div className="mt-8 flex justify-center">
            <Pagination
              ...
              totalPages={kelas8Pagination.lastPage || 1}
