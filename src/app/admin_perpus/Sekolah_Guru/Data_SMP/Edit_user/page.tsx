  ...
  useEffect(() => {
    if (guruSmpDetail) {
      ...
      if (guruSmpDetail.avatar) {
        setPreviewImage(`http://localhost:8000/storage/${guruSmpDetail.avatar}`);
      }
    }
  }, [guruSmpDetail]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (id) {
        ...
        router.push('/admin_perpus/Sekolah_Guru/Data_SMP');
      }
    } catch (error: any) {
        ...
        <div className="bg-white border border-gray-300 rounded-lg p-8 shadow-lg max-w-4xl w-full flex items-center space-x-8">
          ...
          <form onSubmit={handleSubmit} className="grid gap-4 w-full">
            <div>
              ...
              <input type="text" name="username" value={form.username} onChange={handleInputChange} className="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red" />
            </div>

            <div>
              <label className="block text-gray-700 text-sm font-medium mb-2">Email</label>
              <input type="email" name="email" value={form.email} onChange={handleInputChange} className="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red" />
            </div>
            ...
            <div>
              ...
              <input type="text" name="nip" value={form.nip} onChange={handleInputChange} className="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red" />
            </div>

            <div>
              <label className="block text-gray-700 text-sm font-medium mb-2">Sekolah</label>
              <input type="text" name="sekolah" value="SMP" readOnly className="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red bg-gray-100" />
            </div>

            <div className="flex space-x-4">
              <div className="w-1/2">
                <label className="block text-gray-700 text-sm font-medium mb-2">Status</label>
                <input type="text" value="Guru" readOnly className="w-full px-4 py-2 border border-gray-300 rounded-md bg-gray-100 focus:outline-none" />
              </div>

              <div className="w-1/2">
                <label className="block text-gray-700 text-sm font-medium mb-2">Gender</label>
                <select name="gender" value={form.gender} onChange={handleInputChange} className="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red">
            ...
            <div className="flex justify-center mt-8">
              <button type="submit" className="bg-red text-white px-6 py-2 rounded-lg shadow-md hover:bg-OldRed focus:outline-none focus:ring-2 focus:ring-red-400 translate-x-[-50px]">
                Selesai
              </button>
            </div>
          </form>
