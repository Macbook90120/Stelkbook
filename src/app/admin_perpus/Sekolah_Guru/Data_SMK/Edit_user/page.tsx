...
function Page() {
  ...
  const { fetchGuruSmk, guruSmkDetail, updateGuruSmk } = useAuth();
  ...
  useEffect(() => {
    if (guruSmkDetail) {
      setForm({
        id: guruSmkDetail.id || '',
        username: guruSmkDetail.username || '',
        email: guruSmkDetail.email || '',
        password: guruSmkDetail.password || '',
        nip: guruSmkDetail.nip || '',
        gender: guruSmkDetail.gender || '',
        sekolah: 'SMK',
        avatar: guruSmkDetail.avatar || '',
      });

      if (guruSmkDetail.avatar) {
        setPreviewImage(`http://localhost:8000/storage/${guruSmkDetail.avatar}`);
      }
    }
  }, [guruSmkDetail]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (id) {
        ...
        await updateGuruSmk(id, formData);
        alert('Data guru SMK berhasil diperbarui!');
        router.push('/admin_perpus/Sekolah_Guru/Data_SMK');
      }
    } catch (error: any) {
      alert('Gagal memperbarui data guru SMK: ' + (error.message || 'Terjadi kesalahan'));
    }
  };

  const togglePasswordVisibility = () => setShowPassword((prev) => !prev);
  const handleStelkbookClick = () => router.push('/admin_perpus');
  ...
  return (
    <div className="min-h-screen p-8 bg-gray-50">
      <header className="flex justify-between items-center mb-4">
        ...
        <div className="flex-shrink-0 cursor-pointer">
          <Image
            ...
            width={45}
            height={40}
            className="rounded-full -translate-x-5"
          />
        </div>
      </header>
      ...
      <div className="mb-8 flex items-center space-x-2">
        ...
        <p className="text-lg font-semibold text-gray-600">Guru SMK</p>
        <svg className="h-5 w-5 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
        </svg>
      ...
      <div className="flex justify-center">
        <div className="bg-white border border-gray-300 rounded-lg p-8 shadow-lg max-w-4xl w-full flex items-center space-x-8">
          ...
          <div className="relative flex-shrink-0">
            <div onClick={triggerFileInput} className="w-32 h-32 rounded-full overflow-hidden border-2 border-gray-300 cursor-pointer">
              {previewImage ? (
                <Image src={previewImage} alt="User Avatar" width={128} height={128}  quality={100} className="object-cover w-full h-full" />
              ) : (
            ...
            <div className="relative">
              ...
              <div className="relative">
                <input
                  ...
                  value={form.password}
                  onChange={handleInputChange}
                  className="w-full px-4 py-2 border border-gray-300 rounded-md pr-10 focus:outline-none focus:ring-2 focus:ring-red"
                ...
                <button type="button" onClick={togglePasswordVisibility} className="absolute inset-y-0 right-0 pr-3 flex items-center">
                  {showPassword ? (
                    ...
                    <svg className="h-5 w-5 text-gray-500" viewBox="0 0 20 20" fill="currentColor">
                      <path fillRule="evenodd" d="M3.707 2.293a1 1 0 00-1.414 1.414l14 14a1 1 0 001.414-1.414L3.707 2.293z" clipRule="evenodd" />
                    </svg>
                  )}
                </button>
              </div>
