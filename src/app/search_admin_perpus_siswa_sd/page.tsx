...
import Navbar from "@/components/Navbar_Admin_Perpus_SD";
...
const SearchSiswaSD: React.FC = () => {
  ...
  useEffect(() => {
    const getSiswaData = async () => {
      try {
        setIsLoading(true);
        await fetchAllSiswaSd(); // Ambil data siswa SD
      } catch (error) {
  ...
  useEffect(() => {
    if (query && siswaSdData?.length > 0) {
      const results = siswaSdData.filter(
        (siswa: Siswa) =>
    ...
    <div className="min-h-screen p-8 bg-gray-50 overflow-y-auto">
      ...
      <div className="mb-8 flex items-center">
        ...
        <p className="text-xl font-semibold text-left font-poppins translate-y-[-15px]">
          Siswa SD {query && `- Hasil pencarian: "${query}"`}
        </p>
      </div>
      ...
      <div className="relative mb-4">
        <button
          ...
          onClick={() => router.push("/admin_perpus/Create_User_Siswa_SD")}
          title="Create_User"
        >
          +
        </button>
        ...
        <div className="bg-white rounded-lg shadow p-4">
          {filteredSiswa?.length > 0 ? (
              ...
              >
                ...
                <div className="col-span-8 flex justify-end space-x-2">
                  <button
                    ...
                    onClick={() => router.push(`/admin_perpus/Sekolah_Siswa/Data_SD/Edit_user?id=${siswa.id}`)}
                  >
                    <Image
            ...
            <p className="text-gray-500 text-center py-4">
              {query ? "Tidak ada hasil ditemukan untuk pencarian Anda." : "Tidak ada data siswa SD tersedia."}
            </p>
          )}
        </div>
      )}
