...
function Page() {
    ...
    const { fetchKelas3BookById, updateKelas3Book, loading, error } = useBook();
    ...
    useEffect(() => {
        if (id) {
            fetchKelas3BookById(id).then((data: any) => {
                setJudul(data.judul);
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        try {
            await updateKelas3Book(id, formData);
            setShowNotification(true);
            router.push(`/kelasIII_perpus`);
        } catch (err) {
