...
function Page() {
  ...
  // Ambil dan set data guru saat komponen dimount atau ID berubah
  useEffect(() => {
    const loadData = async () => {
      if (id) {
        await fetchGuruSmk(id);
      }
    };
    loadData();
  }, [id, fetchGuruSmk]);

  // Isi form dengan data guru saat data tersedia
  useEffect(() => {
    if (guruSmkDetail) {
      setForm({
        id: guruSmkDetail.id || '',
