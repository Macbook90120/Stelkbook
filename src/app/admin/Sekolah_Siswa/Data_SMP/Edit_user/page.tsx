...
function Page() {
  ...
  const { fetchSiswaSmp, siswaSmpDetail, updateSiswaSmp } = useAuth(); // Ambil fungsi dan state dari AuthContext
  const [form, setForm] = useState({
    ...
    nis: '',
    gender: '',
    sekolah: 'SMP', // Set default sekolah ke "SD"
    kelas: '',
  });
  ...
  useEffect(() => {
    if (id) {
      fetchSiswaSmp(id); // Ambil data siswa spesifik berdasarkan ID
    }
  }, [id, fetchSiswaSmp]);
  ...
  useEffect(() => {
    if (siswaSmpDetail) {
      setForm({
        id: siswaSmpDetail.id || '',
        username: siswaSmpDetail.username || '',
        email: siswaSmpDetail.email || '',
        password: siswaSmpDetail.password || '', // Biarkan kosong untuk keamanan
        nis: siswaSmpDetail.nis || '',
        gender: siswaSmpDetail.gender || '',
        sekolah: 'SMP', // Tetap set ke "SD"
        kelas: siswaSmpDetail.kelas || '',
      });
    }
  }, [siswaSmpDetail]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (id) {
        await updateSiswaSmp(id, form); // Update data siswa
        alert('Data siswa berhasil diperbarui!');
        router.push('/admin/Sekolah_Siswa/Data_SMP'); // Redirect ke halaman data siswa
      }
    } catch (error: any) {
  ...
  return (
    <div className="min-h-screen p-8 bg-gray-50">
      ...
      <div className="mb-8 flex items-center space-x-2">
        ...
        {/* "Guru" Text */}
        <p className="text-lg font-semibold text-gray-600">Siswa SMP</p>
