...
function Page() {
  ...
  useEffect(() => {
    if (siswaSmkDetail) {
      ...
      if (!siswaSmkDetail.avatar) {
        ...
        setPreviewImage(`http://localhost:8000/storage/${siswaSmkDetail.avatar}`);
      }
    }
  }, [siswaSmkDetail]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (id) {
        ...
        alert('Data siswa SMK berhasil diperbarui!');
        router.push('/admin/Sekolah_Siswa/Data_SMK');
      }
    } catch (error: any) {
      alert('Gagal memperbarui data siswa SMK: ' + (error.message || 'Terjadi kesalahan'));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen p-4 md:p-8 bg-gray-50">
      <Head>
        <title>Edit User Siswa SMK</title>
      </Head>
      ...
      <div className="mb-6 md:mb-8 flex items-center space-x-2">
        ...
        <p className="text-sm md:text-lg font-semibold text-gray-700 hover:underline cursor-pointer"
          onClick={() => router.push('/admin/Sekolah_Siswa/Data_SMK')}>
          Siswa SMK
        </p>
        <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 md:h-5 md:w-5 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
