...
function PageContent() {
  ...
  const handlePageChange = (page: number) => {
    ...
    router.push(`/kelasIII?${params.toString()}`);
  };

  useEffect(() => {
    fetchKelas3Books(currentPage);
  }, [fetchKelas3Books, currentPage]);
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'ArrowLeft') {
        ...
        if (kelas3Pagination && currentPage < kelas3Pagination.lastPage) {
  ...
  }, [currentPage, kelas3Pagination]);

  useEffect(() => {
    const processedBooks = kelas3Books.map((book: Book) => {
        ...
        {kelas3Pagination && (
          <div className="mt-8 flex justify-center">
            <Pagination
              ...
              totalPages={kelas3Pagination.lastPage || 1}
