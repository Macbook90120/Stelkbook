...
function Page() {
    ...
    const { fetchKelas9BookById, updateKelas9Book, loading, error } = useBook();
    ...
    useEffect(() => {
        if (id) {
            fetchKelas9BookById(id).then((data: any) => {
                setJudul(data.judul);
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        try {
            await updateKelas9Book(id, formData);
            setShowNotification(true);
            router.push(`/kelasIX_perpus`);
        } catch (err) {
