...
function Kelas4PerpusContent() {
  useAuthMiddleware();
  const router = useRouter();
  const searchParams = useSearchParams();
  const { user } = useAuth();
  const { kelas4Books, kelas4Pagination, loading, error, fetchKelas4Books } = useBook();
  ...
  useEffect(() => {
    fetchKelas4Books(currentPage);
  }, [fetchKelas4Books, currentPage]);

  const handlePageChange = (page: number) => {
    const params = new URLSearchParams(searchParams.toString());
    params.set('page', page.toString());
    router.push(`/kelasIV_perpus?${params.toString()}`);
  };
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'ArrowLeft') {
        ...
        if (kelas4Pagination && currentPage < kelas4Pagination.lastPage) {
  ...
  }, [currentPage, kelas4Pagination]);

  useEffect(() => {
    if (kelas4Books) {
      const filteredBooks = kelas4Books.filter((book: any) => {
  ...
  }, [kelas4Books, sortOption, activeFilters]);
      ...
      <main className="pt-24 px-4 sm:px-8 flex-grow flex flex-col pb-8">
        <div className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div className="flex flex-col gap-1">
            <p className="text-xl font-semibold font-poppins">Buku Kelas IV</p>
            <p className="text-sm text-gray-500">Menampilkan {displayBooks.length} buku</p>
          </div>
          <div className="flex flex-wrap items-center gap-3">
            <FilterCheckbox books={kelas4Books || []} onFilterChange={setActiveFilters} />
        ...
        {kelas4Pagination && (
          <div className="mt-8 flex justify-center">
            <Pagination
              ...
              totalPages={kelas4Pagination.lastPage || 1}
