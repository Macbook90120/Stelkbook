...
function Page() {
    ...
    const { fetchKelas4BookById, updateKelas4Book, loading, error } = useBook();
    ...
    useEffect(() => {
        if (id) {
            fetchKelas4BookById(id).then((data: any) => {
                setJudul(data.judul);
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        try {
            await updateKelas4Book(id, formData);
            setShowNotification(true);
            router.push(`/kelasIV_perpus`);
        } catch (err) {
