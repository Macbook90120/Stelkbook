...
function Page() {
    ...
    const { fetchKelas12BookById, updateKelas12Book, loading, error } = useBook();
    ...
    useEffect(() => {
        if (id) {
            fetchKelas12BookById(id).then((data: any) => {
                setJudul(data.judul);
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        try {
            await updateKelas12Book(id, formData);
            setShowNotification(true);
            router.push(`/kelasXII_perpus`);
        } catch (err) {
