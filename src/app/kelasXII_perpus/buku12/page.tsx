...
const Page: React.FC = () => {
  ...
  const { fetchKelas12BookById, deleteBookKelas12, getBookPdfUrl } = useBook(); // ✅ Ambil fungsi getBookPdfUrl
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await fetchKelas12BookById(bookId);
        setBook(data);
    ...
    fetchData();
  }, [bookId, fetchKelas12BookById, getBookPdfUrl]);

  const handleDeleteBook = async (id: number) => {
    try {
      await deleteBookKelas12(id);
      ...
      <div className="flex flex-col lg:flex-row gap-8 items-center">
        ...
        <div className="flex flex-col items-center lg:items-start">
          ...
          <div className="mt-4 flex flex-col gap-2">
            <button
              onClick={() => router.push(`/kelasXII_perpus/buku12/Edit_Buku?id=${book.id}`)}
