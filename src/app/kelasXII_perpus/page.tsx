...
function Kelas12PerpusContent() {
  useAuthMiddleware();
  const router = useRouter();
  const searchParams = useSearchParams();
  const { user } = useAuth();
  const { kelas12Books, kelas12Pagination, loading, error, fetchKelas12Books } = useBook();
  ...
  useEffect(() => {
    fetchKelas12Books(currentPage);
  }, [fetchKelas12Books, currentPage]);

  const handlePageChange = (page: number) => {
    const params = new URLSearchParams(searchParams.toString());
    params.set('page', page.toString());
    router.push(`/kelasXII_perpus?${params.toString()}`);
  };
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'ArrowLeft') {
        ...
        if (kelas12Pagination && currentPage < kelas12Pagination.lastPage) {
  ...
  }, [currentPage, kelas12Pagination]);

  useEffect(() => {
    if (kelas12Books) {
      const filteredBooks = kelas12Books.filter((book: any) => {
      ...
      const processedBooks = filteredBooks.map((book: any) => {
        ...
        return {
          ...
          judul: book.judul,
          cover: coverUrl,
          path: `/kelasXII_perpus/buku12?id=${book.id}`,
  ...
  }, [kelas12Books, sortOption, activeFilters]);
      ...
      <main className="pt-24 px-4 sm:px-8 flex-grow flex flex-col pb-8">
        <div className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div className="flex flex-col gap-1">
            <p className="text-xl font-semibold font-poppins">Buku Kelas XII</p>
            <p className="text-sm text-gray-500">Menampilkan {displayBooks.length} buku</p>
          </div>
          <div className="flex flex-wrap items-center gap-3">
            <FilterCheckbox books={kelas12Books || []} onFilterChange={setActiveFilters} />
        ...
        {kelas12Pagination && (
          <div className="mt-8 flex justify-center">
            <Pagination
              ...
              totalPages={kelas12Pagination.lastPage || 1}
